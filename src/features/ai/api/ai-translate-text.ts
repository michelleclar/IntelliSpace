...
const aiTranslateTextReplyFormat = (aiReply: string) => {
  return JSON.stringify(
    aiReplyFormatToDelta({ aiReply, modelName: "Translate" }),
  );
};

const aiTranslateText = async ({ content, token }: AiRequestProps) => {
  if (content.replace(/<(.|\n)*?/g, "").trim().length === 0) {
    return void 0;
  }
  const message = {
  ...
  const mutate = useCallback(
    async (values: AiRequestProps, options?: AiOptions) => {
      try {
        ...
        const data = await aiTranslateText(values);
        ...
        const _body = aiTranslateTextReplyFormat(aiReply);
